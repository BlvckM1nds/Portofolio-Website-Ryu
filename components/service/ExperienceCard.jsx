  ...
  return (
    <div className="flex-1 flex flex-col justify-center gap-6 group">
      <div className="w-full flex justify-between items-center">
        ...
        <Link
          href={href}
          className="w-[70px] aspect-square rounded-full bg-secondary group-hover:bg-accent transition-all duration-300 flex items-center justify-center hover:-rotate-45"
          target="_blank"
        >
          <BsArrowDownRight className="text-primary text-3xl" />
        </Link>
      </div>
      <h2 className="text-[42px] font-bold leading-none text-secondary group-hover:text-accent transition-all duration-300">
        {title}
      </h2>
      <p className="text-secondary/60">{description}</p>
      <hr className="border-secondary/20 w-full" />
    </div>
  );
};